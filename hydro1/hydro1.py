...
class Ppt2Net:
    ...
    def ppt2geojson(self, ppt, clip=True):
        '''
        Takes one point in GeoJSON and retuns the snapped point in GeoJSON
        format and the accompanied upstream river network as a dictionary. 
        The catchment polygon is also returned. If clip is False the stream
        line is not shortened to the snapped point.
        
        Args:
          ppt(dict)     : The pouring point in GeoJSON format
          clip(boolean) : Shorten the stream line to the snapped point or not
